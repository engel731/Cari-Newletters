  ...
  class StreetManager
  {
    ...
    public function search($keypress) {
      ...
      $resultats = $wpdb->get_results(
        $wpdb->prepare(
          ...
          FROM {$wpdb->prefix}cinor_crom_street_listing 
    ...
    public function add(Street $street)
    {
      ...
      if($this->notExistItem($street)) {
        ...
        $wpdb->insert(
          "{$wpdb->prefix}cinor_crom_street_listing",
    ...
    public function notExistItem(Street $street) 
    {
      ...
      $row = $wpdb->get_row(
        $wpdb->prepare(
          "SELECT id FROM {$wpdb->prefix}cinor_crom_street_listing WHERE ref_calendrier = %s AND intitule_voie = %s AND quartier = %s",
    ...
    public function idDoesExist($id)
    {
      ...
      $row = $wpdb->get_row(
        $wpdb->prepare(
          "SELECT id FROM {$wpdb->prefix}cinor_crom_street_listing WHERE id = %d",
          $id
        )
      );
    ...
    public function create() {
      ...
      $wpdb->query(
        "CREATE TABLE IF NOT EXISTS {$wpdb->prefix}cinor_crom_street_listing (
    ...
    public function cleanWholeTable() {
      ...
      $wpdb->query("TRUNCATE TABLE {$wpdb->prefix}cinor_crom_street_listing"); 
    }

    public function delete() {
      $wpdb = $this->_wpdb;
      $wpdb->query("DROP TABLE IF EXISTS {$wpdb->prefix}cinor_crom_street_listing");
    }
